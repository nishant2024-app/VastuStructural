...
import { Phone, ArrowRight, Shield, Building2, Loader2 } from "lucide-react";
...
export default function ContractorLoginPage() {
    ...
    const handleOtpSubmit = async (e: React.FormEvent) => {
        ...
        if (otpValue === "123456") {
            localStorage.setItem("contractorAuth", JSON.stringify({ phone, loggedIn: true }));
            ...
            <div className="w-full max-w-md relative z-10">
                    ...
                    {step === "phone" ? (
                        <form onSubmit={handlePhoneSubmit} className="space-y-6">
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-14 pr-4 py-4 bg-background border border-border rounded-xl text-foreground text-lg placeholder:text-foreground/30 focus:outline-none focus:border-accent transition-colors"
                                    />
                                </div>
                            </div>
                            ...
                            <button
                                type="submit"
                                disabled={isLoading}
                                className="w-full py-4 bg-gradient-to-r from-accent to-primary text-background rounded-xl font-bold text-lg flex items-center justify-center gap-2 hover:shadow-[0_4px_30px_-5px] hover:shadow-accent/40 transition-all disabled:opacity-50"
                            >
                                {isLoading ? (
                                ...
                                <div className="flex gap-2 justify-center">
                                    {otp.map((digit, idx) => (
                                        <input
                                            ...
                                            className="w-12 h-14 text-center text-xl font-bold bg-background border border-border rounded-xl text-foreground focus:outline-none focus:border-accent transition-colors"
                                            maxLength={1}
                                        />
                                    ))}
                                </div>
                ...
                <div className="mt-6 text-center">
                    ...
                    <p className="text-foreground/40 text-sm">
                        Only approved contractors can login.
                    </p>
                    <Link
                        href="/partners/apply"
